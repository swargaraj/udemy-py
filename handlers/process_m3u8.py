...
def download_and_merge_m3u8(
    m3u8_file_url, download_folder_path, title_of_output_mp4, task_id, progress
):
    progress.update(
        task_id,
        description=f"Downloading Stream {remove_emojis_and_binary(title_of_output_mp4)}",
        completed=0,
    )
    ...
    for pl in playlists:
        ...
        if resolution and (
            resolution[0] * resolution[1] > max_resolution[0] * max_resolution[1]
        ):
    ...
    if not highest_quality_playlist:
        progress.console.log(
            f"No valid playlists {remove_emojis_and_binary(title_of_output_mp4)} ✕"
        )
    ...
    merge_segments_into_mp4(
        m3u8_file_path, download_folder_path, title_of_output_mp4, task_id, progress
    )


def merge_segments_into_mp4(
    m3u8_file_path, download_folder_path, output_file_name, task_id, progress
):
    output_path = os.path.dirname(download_folder_path)

    progress.update(
        task_id,
        description=f"Merging segments {remove_emojis_and_binary(output_file_name)}",
        completed=0,
    )
    ...
    if stderr or process.returncode != 0:
        progress.console.log(
            f"[red]Error Merging {remove_emojis_and_binary(output_file_name)}[/red] ✕"
        )
        progress.remove_task(task_id)
        return

    progress.console.log(
        f"[green]Downloaded {remove_emojis_and_binary(output_file_name)}[/green] ✓"
    )
    progress.remove_task(task_id)
