...
from lib.helpers import remove_emojis_and_binary, timestamp_to_seconds


def download_and_merge_mpd(
    mpd_file_url,
...
def process_mpd(
    ...
    if stderr_nm3u8dl or process_nm3u8dl.returncode != 0:
        progress.console.log(
            f"[red]Error Downloading Segments {remove_emojis_and_binary(output_file_name)}[/red] ✕"
        )
    ...
    if not mp4_files or not m4a_files:
        progress.console.log(
            f"[red]Missing Video and Audio files {output_file_name}[/red] ✕"
        )
        progress.remove_task(task_id)
        return
    ...
    progress.update(
        task_id,
        description=f"Merging Video and Audio {remove_emojis_and_binary(output_file_name)}",
        completed=0,
    )
    ...
    if stderr_ffmpeg or process_ffmpeg.returncode != 0:
        progress.console.log(
            f"[red]Error Merging Video and Audio files {remove_emojis_and_binary(output_file_name)}[/red] ✕"
        )
        progress.remove_task(task_id)
        return
