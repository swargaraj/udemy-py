...
def download_and_merge_m3u8(m3u8_file_url, download_folder_path, title_of_output_mp4, task_id, progress):
    progress.update(task_id,  description=f"Downloading Stream {remove_emojis_and_binary(title_of_output_mp4)} [HLS]", completed=0)
    ...
    if not highest_quality_playlist:
        progress.console.log(f"No valid playlists {remove_emojis_and_binary(title_of_output_mp4)} [HLS] ✕")
